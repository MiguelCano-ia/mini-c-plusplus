...
class SemanticAnalyzer(Visitor):
  ...
  @multimethod
  def visitObjectDecl(self, node: ObjectDecl):
  ...
  def visit(self, node: PrintStmt):
    formatStrType = self.visit(node.expr)
    if formatStrType not in {'string'}:
      self.errors.append(f"Error: The argument of 'print' must be of type 'string', found '{formatStrType}'")
      return
    
    formatStr = getattr(node.expr, 'value', None)
    if formatStr is None:
      self.errors.append(f"Error: The argument of 'print' must be a constant string")
      return

    formatSpecifiers = {
      '%d': 'int', '%i': 'int', '%u': 'unsigned int', '%f': 'float',
      '%c': 'char', '%s': 'string'
    }
    
    specifiers = re.findall(r'%[diufcs]', formatStr)
    
    if len(specifiers) != len(node.args):
      self.errors.append(f"Error: Number of format specifiers does not match the number of arguments")
      return
    
    for i, specifier in enumerate(specifiers):
      expectedType = formatSpecifiers.get(specifier)
      if not expectedType:
      ...
      if argType != expectedType:
        self.errors.append(f"Error: Argument {i+1} must be of type '{expectedType}', found '{argType}'")
